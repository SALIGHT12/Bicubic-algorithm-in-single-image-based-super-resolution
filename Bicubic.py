...
f = open('Bicubic_PSNR_SSIM_RESULTS.txt', 'w')
...
for filename in os.listdir(folder_path):
    ...
    if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".jpeg"):
        ...
        Ilowres = cv2.resize(Ireference, new_size, interpolation=cv2.INTER_CUBIC)
        ...
        IBicubic  = cv2.resize(Ilowres, [nrows, ncols], interpolation=cv2.INTER_CUBIC)
        # save 
        cv2.imwrite(os.path.join(result_path, filename), IBicubic  )
        psnr_x_ = compare_psnr(Ireference, IBicubic  )
        ssim_x_ = compare_ssim(Ireference, IBicubic  , win_size=11, multichannel=True)
        score2= brisque.score(IBicubic)
...
# # Display the figure
# plt.show()
# cv2.imwrite('C:/Users/Soumia/Desktop/Projet_Messali_Zoubeida/Soumia_programs/nearst/IBicubic  .jpg', IBicubic  )
...
IBicubic   = cv2.imread(os.path.join(result_path,str(1) + ".jpg"))
axes[0].imshow(Ireference[:,:,::-1])
axes[1].imshow(Ilowres[:,:,::-1])
axes[2].imshow(IBicubic [:,:,::-1])
